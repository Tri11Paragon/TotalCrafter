...
import com.brett.engine.data.datatypes.AnimatedFrame;
import com.brett.engine.data.datatypes.AnimatedMesh;
import com.brett.engine.data.datatypes.Animation;
import com.brett.engine.data.datatypes.Bone;
import com.brett.engine.data.datatypes.Material;
import com.brett.engine.data.datatypes.Mesh;
import com.brett.engine.data.datatypes.Node;
import com.brett.engine.data.datatypes.VertexWeight;
...
public class AnimMeshesLoader {
	...
	public static AnimatedMesh loadAnimGameItem(String resourcePath, String texturesDir) throws Exception {
	...
	public static AnimatedMesh loadAnimGameItem(String resourcePath, String texturesDir, int flags) throws Exception {
		...
		for (int i = 0; i < numMaterials; i++) {
			...
			ModelLoader.processMaterial(aiMaterial, materials, texturesDir);
		}
		...
		AnimatedMesh item = new AnimatedMesh(meshes, animations);
	...
	private static Mesh processMesh(AIMesh aiMesh, List<Material> materials, List<Bone> boneList) {
		...
		ModelLoader.processVertices(aiMesh, vertices);
		ModelLoader.processNormals(aiMesh, normals);
		ModelLoader.processTextCoords(aiMesh, textures);
		ModelLoader.processIndices(aiMesh, indices);
		...
		Mesh mesh = new Mesh(Utils.listToArrayF(vertices), Utils.listToArrayF(textures), Utils.listToArrayF(normals),
				Utils.listToArrayI(indices), Utils.listToArrayI(boneIds), Utils.listToArrayF(weights));
		Material material;
